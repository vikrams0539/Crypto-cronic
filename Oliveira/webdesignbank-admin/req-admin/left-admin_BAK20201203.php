<div class='list-group admin-left'>
	<?php if($usertype['type']!=='admin')
			...
			if($admin_url==$website_domain."webdesignbank-admin/") 
			{
				$li="li_active"; 
			}
			else
	...
	<ul>
		...
		<div class='child-index index' <?=$display_block_text;?>>
			<ol type='1'>				
				<?php
					...
					$table_index=$fun_obj->commonSelect_table('cms_pages','page_ID^page_name^filename',"WHERE flag=''");
					while($run_table=mysqli_fetch_array($table_index,MYSQLI_ASSOC))
					{
						...
						echo"<li class='$li_active'><a href='".$admin_url."index.php?pageID=$pageID'>".$page__name."</a>";
						if($usertype['type']=='admin')
						{
							echo"<span data-edit='$pageID'>edit</span>";
						}
						$sub_menu=$fun_obj->commonSelect_table("cms_pages","page_ID^page_name^filename^page_order^for_menu^flag","WHERE flag='$pageID' AND sub_menu_active='y'");
							echo"<ul>";
							while($sub_run=mysqli_fetch_array($sub_menu))
								{
									...
									echo"<li class='$li_active'><a href='".$admin_url."index.php?pageID=".$sub_run['page_ID']."'>".$sub_run['page_name']."</a>";
									if($usertype['type']=='admin')
										{
											echo"<span data-edit='".$sub_run['page_ID']."'>edit</span>";
										}
										
									$sub_sub_menu=$fun_obj->commonSelect_table("cms_pages","page_ID^page_name^filename^page_order^for_menu^flag","WHERE flag='$sub_pageid' AND sub_menu_active='y'");
									...
									<ul>";
										while($sub_sub_menu_run=mysqli_fetch_array($sub_sub_menu))
										{
											...
											if($sub_sub_pageid!='')
											{
											echo"<li class='$li_active'><a href='".$admin_url."index.php?pageID=".$sub_sub_menu_run['page_ID']."'>".$sub_sub_menu_run['page_name']."</a></li>";
											}
										}
									echo"</ul>
									</li>";
			...
			<ol type='2'>					
					 ...
					 while($run_galPage=mysqli_fetch_array($galleryPage))
					 {
						...
						if(strlen($page_name)>22)
						{
						 ...
						 echo"
						 <li class='$li_active'>
						<a href='".$admin_url."index.php?upload_img=$ID'>".$page_name."</a>
						&emsp;<a class='btn-sm btn-danger text-white' href='".$admin_url."index.php?upload_img=$ID'>Upload/Del</a>";
						...
						<a href='".$admin_url."index.php?upload_img=$cat_pagename_id'>".$cat_pagename."</a>
						&emsp;</li>";
						}
						echo"</ul>";
					echo"</li>";
		...
		<li data-index-filter="miscellaneous" class="<?=$li_hide?>">
			<h4><i class='fa fa-file-text-o' aria-hidden='true'></i>&nbsp;miscellaneouse</h4>
		</li> 
		<div class='child-index miscellaneous' <?=$display_block_img;?>>
			<ol type='2'>					
				<li class='<?=$li_hide?>'>
					<a href="<?=$admin_url?>miscellaneouse.php">add page</a>
				</li>
			</ol>
		</div>		
	</ul>
